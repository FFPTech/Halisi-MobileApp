...
export const handleLoginAPI = async (email) => {
  try {
    const response = await axios.post(
      'https://hal-liv-qua-san-fnapp-v1.azurewebsites.net/api/verifyuseremailid',
      { email_id: email, env: 'Qua' }
    )
...
export const getCompanyData = async (
  ...
  try {
    const response = await axios.post(
      'https://hal-liv-qua-san-fnapp-v1.azurewebsites.net/api/getCompanyinformation',
      data
    )
    const res = response.data
    // console.log(res);
